...
export class MongoDbSchemaGenerator extends BaseGenerator {
  ...
  private async generateDatabaseAwareRepository(
    ...
    try {
      ...
      const repoTemplate = await this.templateEngine.render(
        'infrastructure/database-aware-repository.ts.ejs',
  ...
  private async ensureDatabaseServiceExists(context: GenerationContext, result: GenerationResult): Promise<void> {
    try {
      ...
      if (!exists) {
        ...
        const serviceTemplate = await this.templateEngine.render(
          'infrastructure/database-service.ts.ejs',
          context.templateData
        );
...
export interface DatabaseConnectionConfig {
  ...
  private async createDatabaseConnectionFactory(databaseDir: string, result: GenerationResult): Promise<void> {
    ...
    if (!exists) {
      const factoryContent = `import { DatabaseConnection, DatabaseConnectionConfig } from './database-connection.interface';
...
export class DatabaseConnectionFactory {
  ...
  private async createMongoDbConnection(databaseDir: string, result: GenerationResult): Promise<void> {
    ...
    if (!exists) {
      // MongoDB connection implementation should ideally be a template too
      this.logger.info(`📦 MongoDB connection implementation needed: ${connectionPath}`);
      this.logger.info(`💡 Using existing MongoDB connection from test project as reference`);
    }
  }
